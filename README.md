...
4. 发布。`npm run release drafts/xxx.glsl` 生成产物并刷新清单

```bash
cp drafts/_temple.glsl drafts/xxx.glsl   # 建草稿
npm run release drafts/xxx.glsl          # 发布：生成产物 + 刷新清单
...
```
drafts/xxx.glsl ──release──▶ shader/xxx.shader.js ──▶ shader/manifest.js
（开发中，不提交）        （产物，提交）           （清单，自动刷新，提交）
        ▲                          │
        └────────extract───────────┘
```

`drafts/` 是开发中的 GLSL，**默认不提交**，只有显式 `release` 才会成为内置 shader。

| 命令 | 作用 |
|---|---|
| `npm run release` | 不带参数 = 显示状态（只读） |
| `npm run release drafts/x.glsl` | 发布草稿，自动刷新清单（可传多个） |
| `npm run release:status` | 查看状态 |
| `npm run release:all` | 重组装已发布过的草稿（不会新增） |
| `npm run release:refresh` | 只重建清单（手工增删产物文件后用） |
| `npm run release:check` | 草稿改动后未重新发布则退出码 1 |
| `npm run drafts:extract -- shader/x.js` | 从产物反向导出可编辑草稿 |
| `npm run shaders:check` | 总校验（CI 用） |
...
| 写法 | 结果 |
|---|---|
| `npm run release drafts/a.glsl` | ✓ 纯位置参数，无需 `--` |
| `npm run release:all` | ✓ 固定动作走子命令，无需 `--` |
| `npm run release -- drafts/a.glsl --label X` | ✓ 有 flag，加了 `--` |
| `npm run release drafts/a.glsl --label X` | ✗ `X` 被当成文件名 |
| `npm run drafts:extract shader/x.js --force` | ✗ 实际没有 `--force` |

flag 写在文件名后面一样会被吞。不带参数时 `release` 是只读的，所以漏写 `--` 只会白跑一次，不会误改文件。

多草稿发布时，只要有一个被拦截就整体退出码 1（不会误判为全部成功），已成功的部分仍会写盘并同步刷新清单。

### 草稿与产物的六种状态

用三个哈希（草稿当前值 / 产物记录指纹 / 产物内代码实际值）判断哪边动过：

| 状态 | 含义 | `release` 行为 | `--check` |
|---|---|---|---|
| `未发布` | 产物不存在 | 新建 | 通过 |
| `未追踪` | 产物非本工具生成（历史 shader） | 拒绝，需 `--force` 接管 | 通过 |
| `已同步` | 两边都没动 | 无操作 | 通过 |
| `草稿有更新` | 只有草稿改了 | 用草稿更新产物 | **失败** |
| `产物手工改` | 只有产物被手工编辑 | **拒绝**（会破坏手工改动） | 仅警告 |
| `冲突` | 两边都改过 | **拒绝**，需人工合并 | **失败** |

因此：人工调优产物不会让 CI 报警，AI 跑 `release` 也绝不会静默覆盖手工改动。把手工改动同步回草稿：

```bash
npm run drafts:extract -- shader/xxx.shader.js --force   # 导回草稿
npm run release drafts/xxx.glsl                          # 刷新指纹
```
...
- 产物由工具生成；**人可以直接手工编辑**（工具会识别并保护），AI 则一律走 `drafts/*.glsl` → `release`。
...
├── scripts/                    # 发布 / 导出 / 清单工具（零依赖）
│   ├── release-shader.js, extract-shader.js, gen-shader-manifest.js
│   └── lib/shader-build.js
├── img/                        # 文档截图
